...
```mermaid
flowchart TB
    subgraph Container["📦 CONTAINER (Instance Management Hub)"]
        subgraph Stores["🔄 STORES (Reactive)"]
            direction LR
            userStore["<b>userStore</b><br/>state: profile, token<br/>actions: login, logout"]
            cartStore["<b>cartStore</b><br/>state: items, total<br/>actions: add, checkout"]
            uiStore["<b>uiStore</b><br/>state: theme, modal<br/>actions: toggle, setTheme"]
            cartStore -.->|get| userStore
        end
        
        subgraph Services["⚙️ SERVICES (Non-reactive)"]
            direction LR
            apiService["<b>apiService</b><br/>get(), post()"]
            authService["<b>authService</b><br/>getToken(), refresh()"]
            logService["<b>logService</b><br/>info(), error()"]
        end
        
        userStore --> apiService
        cartStore --> authService
    end
    
    C1["🧩 Component<br/>useStore()"]
    C2["🧩 Component<br/>useStore()"]
    C3["🧩 Component<br/>useStore()"]
...
sequenceDiagram
    ...
    Note over Store,Proxy: 1️⃣ SETUP PHASE
    Store->>Proxy: Wrap state in Proxy
    
    Note over Proxy,Component: 2️⃣ RENDER PHASE
    Component->>Proxy: useStore() selector runs
    Proxy-->>Proxy: Record: state.profile.name accessed
    Proxy->>Component: Return { name: "John" }
    Note right of Proxy: Dependencies: ["profile", "name"]
    
    Note over Store,Component: 3️⃣ UPDATE PHASE
    Store->>Store: state.profile.name = "New Name"
    Store->>Proxy: Check subscribers for "profile.name"
    Proxy->>Component: Notify only affected components
