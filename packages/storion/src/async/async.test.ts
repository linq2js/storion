...
describe("async", () => {
  ...
  describe("ensure", () => {
    it("should dispatch if deps changed", async () => {
      const [focus] = createMockFocus(async.fresh<string>());
      const handler = vi.fn(async (_ctx, id: string) => `data-${id}`);

      const { ensure } = async(focus, handler);

      await ensure(["dep1"], "id1");
      expect(handler).toHaveBeenCalledTimes(1);

      await ensure(["dep2"], "id2");
      expect(handler).toHaveBeenCalledTimes(2);
    });

    it("should skip dispatch if deps are same", async () => {
      const [focus, { getState }] = createMockFocus(async.fresh<string>());
      const handler = vi.fn(async (_ctx, id: string) => `data-${id}`);
      ...
      expect(getState().data).toBe("data-id1");
    });

    it("should dispatch if args changed even if deps are same", async () => {
      const [focus, { getState }] = createMockFocus(async.fresh<string>());
      const handler = vi.fn(async (_ctx, id: string) => `data-${id}`);
      ...
      // Same deps, different args → should dispatch
      await ensure(["dep1"], "id2");
      expect(handler).toHaveBeenCalledTimes(2);
      expect(getState().data).toBe("data-id2");
    });

    it("should skip dispatch only if both deps and args are same", async () => {
      const [focus] = createMockFocus(async.fresh<string>());
      const handler = vi.fn(async (_ctx, a: string, b: number) => `${a}-${b}`);

      const { ensure } = async(focus, handler);

      await ensure(["extra"], "a", 1);
      expect(handler).toHaveBeenCalledTimes(1);
      ...
      // Deps same, args different → dispatch
      await ensure(["extra"], "a", 2);
      expect(handler).toHaveBeenCalledTimes(2);

      // Deps different, args same → dispatch
      await ensure(["other"], "a", 2);
      expect(handler).toHaveBeenCalledTimes(3);
    });
  });
