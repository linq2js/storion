...
describe.each(wrappers)("useStore ($mode mode)", ({ render, renderHook }) => {
  ...
  describe("SelectorContext.create()", () => {
    it("should create parameterized factory instances", () => {
      const createLogger = (
        _resolver: any,
        namespace: string,
        level: number
      ...
      const { result } = renderHook(
        () =>
          useStore(({ create }) => {
            const logger = create(createLogger, "test-component", 1);
            return { logger };
          }),
        { wrapper: createWrapper(stores) }
      );

      expect(result.current.logger.namespace).toBe("test-component");
      expect(result.current.logger.level).toBe(1);
      expect(result.current.logger.log("hello")).toBe("[test-component:1] hello");
    });

    it("should create fresh instances on each render", () => {
      let instanceCount = 0;
      const createService = (_resolver: any, id: string) => ({
        id,
      ...
      const { result, rerender } = renderHook(
        () =>
          useStore(({ create }) => {
            const service = create(createService, "svc");
            return { service };
          }),
        { wrapper: createWrapper(stores) }
      );

      const firstInstanceId = result.current.service.instanceId;

      rerender();

      // New instance on re-render (fresh each time)
      expect(result.current.service.instanceId).not.toBe(firstInstanceId);
    });

    it("should have access to container for resolving dependencies", () => {
      const configFactory = () => ({ baseUrl: "http://api.example.com" });

      const createClient = (resolver: any, timeout: number) => {
        const config = resolver.get(configFactory);
        return { url: config.baseUrl, timeout };
      };
      ...
      const { result } = renderHook(
        () =>
          useStore(({ create }) => {
            const client = create(createClient, 3000);
            return { client };
          }),
        { wrapper: createWrapper(stores) }
      );

      expect(result.current.client.url).toBe("http://api.example.com");
      expect(result.current.client.timeout).toBe(3000);
    });
  });
